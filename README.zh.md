...
这是给 AI agent 使用的通用研究安全规则包，用来约束项目中的删改数据、恢复数据、清理命令和结果覆盖。它的目标不是替你做实验，而是让 Claude Code、Codex 或其他 AI 在动手前先检查：会不会误删数据、覆盖结果，或者在路径和备份不清楚时执行高风险操作。
...
```text
请使用 research-guardrails 规则。先判断本次任务属于 text-only、plotting-only、postprocessing-only、experiment-changing 还是 destructive-risk，再继续；涉及删除、覆盖、同步或清理时必须先做路径、备份和影响范围检查。
```
...
一般不需要先填复杂配置。默认受保护目录和私密目录清单以 `references/configuration.md` 为准；默认用“移动到隔离区”替代永久删除。
...
核心原则：AI 只能在路径清楚、备份清楚、风险分类清楚之后行动。
